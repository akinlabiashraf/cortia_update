...
/// <reference path="data-attr-helper.js"/>
{
  ...
  const parseOptions = (attr, el, useDefaults = false) => {
    const options = {};
    ((useDefaults ? defaultOptions + ";" : "") + attr).split(/(?<!\\);/).filter(item => item.trim()).map(item => item.split(":").map(w => w.trim())).forEach(([key, value]) => {
  ...
  /** @type {Record<string, number>} */  const breakpoints = {
    xs: 480,
    s: 640,
    m: 960,
    l: 1200,
  ...
  const initSwiper = (div, optionsAttr = "data-uc-swiper") => {
    ...
    const ready = () => {
      ...
      swiper.update();
      document.addEventListener("DOMContentLoaded", () => swiper.update(), {
        once: true
      });
